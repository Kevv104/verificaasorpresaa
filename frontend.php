<?php
// frontend.php — Route frontend (dashboard, pagine query)
// Questo file viene incluso da index.php DOPO la creazione di $app e $pdo.
// Non ha require/autoload proprio perché li eredita da index.php.

use Psr\Http\Message\ResponseInterface as Response;
use Psr\Http\Message\ServerRequestInterface as Request;

// ══════════════════════════════════════════════════════════════════════════
// FUNZIONI HELPER FRONTEND
// ══════════════════════════════════════════════════════════════════════════
...
function renderPage(Response $res, string $activeQuery, string $pageContent): Response {
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    function addPezzo() {
        openDlg("<button class=\"dlg-close\" onclick=\"closeDlg()\">\u2715</button><h3>Aggiungi Pezzo</h3><div class=\"ff\"><label>Nome *</label><input id=\"f-pn\" placeholder=\"Nome pezzo\"/></div><div class=\"ff\"><label>Colore</label><input id=\"f-co\" placeholder=\"es. rosso\"/></div><div class=\"ff\"><label>Peso</label><input id=\"f-pe\" type=\"number\" step=\"0.1\"/></div><div class=\"dlg-actions\"><button class=\"btn-sm\" onclick=\"closeDlg()\">Annulla</button><button class=\"btn-sm btn-accent\" onclick=\"savePezzo()\">Salva</button></div>");
    }
    function editPezzo(p) {
        openDlg("<button class=\"dlg-close\" onclick=\"closeDlg()\">\u2715</button><h3>Modifica Pezzo #" + p.pid + "</h3><div class=\"ff\"><label>Nome *</label><input id=\"f-pn\" value=\"" + p.pnome + "\"/></div><div class=\"ff\"><label>Colore</label><input id=\"f-co\" value=\"" + (p.colore||"") + "\"/></div><div class=\"ff\"><label>Peso</label><input id=\"f-pe\" type=\"number\" step=\"0.1\" value=\"" + (p.peso||"") + "\"/></div><div class=\"dlg-actions\"><button class=\"btn-sm\" onclick=\"closeDlg()\">Annulla</button><button class=\"btn-sm btn-accent\" onclick=\"savePezzo(" + p.pid + ")\">Salva</button></div>");
    }
    async function savePezzo(pid=null) {
        const b = { pnome: document.getElementById("f-pn").value.trim(), colore: document.getElementById("f-co").value.trim()||null, peso: document.getElementById("f-pe").value||null };
        if (!b.pnome) { showAlert("Il nome è obbligatorio","err"); return; }
        try { await apiCall(pid?"/api/pezzi/"+pid:"/api/pezzi", pid?"PUT":"POST", b); closeDlg(); showAlert(pid?"Pezzo aggiornato":"Pezzo creato"); pzPg=1; loadPezzi(); } catch(e) { showAlert(e.message,"err"); }
    }
    async function delPezzo(pid) {
        if (!confirm("Eliminare il pezzo #"+pid+"?")) return;
        try { await apiCall("/api/pezzi/"+pid,"DELETE"); showAlert("Pezzo eliminato"); loadPezzi(); } catch(e) { showAlert(e.message,"err"); }
    }
    ...
    function addFornitore() {
        openDlg("<button class=\"dlg-close\" onclick=\"closeDlg()\">\u2715</button><h3>Aggiungi Fornitore</h3><div class=\"ff\"><label>Nome *</label><input id=\"f-fn\" placeholder=\"Nome fornitore\"/></div><div class=\"ff\"><label>Citt\u00e0</label><input id=\"f-ci\" placeholder=\"es. Milano\"/></div><div class=\"dlg-actions\"><button class=\"btn-sm\" onclick=\"closeDlg()\">Annulla</button><button class=\"btn-sm btn-accent\" onclick=\"saveFornitore()\">Salva</button></div>");
    }
    function editFornitore(f) {
        openDlg("<button class=\"dlg-close\" onclick=\"closeDlg()\">\u2715</button><h3>Modifica Fornitore #" + f.fid + "</h3><div class=\"ff\"><label>Nome *</label><input id=\"f-fn\" value=\"" + f.fnome + "\"/></div><div class=\"ff\"><label>Citt\u00e0</label><input id=\"f-ci\" value=\"" + (f.citta||"") + "\"/></div><div class=\"dlg-actions\"><button class=\"btn-sm\" onclick=\"closeDlg()\">Annulla</button><button class=\"btn-sm btn-accent\" onclick=\"saveFornitore(" + f.fid + ")\">Salva</button></div>");
    }
    async function saveFornitore(fid=null) {
        const b = { fnome: document.getElementById("f-fn").value.trim(), citta: document.getElementById("f-ci").value.trim()||null };
        if (!b.fnome) { showAlert("Il nome è obbligatorio","err"); return; }
        try { await apiCall(fid?"/api/fornitori/"+fid:"/api/fornitori", fid?"PUT":"POST", b); closeDlg(); showAlert(fid?"Fornitore aggiornato":"Fornitore creato"); fnPg=1; loadFornitori(); } catch(e) { showAlert(e.message,"err"); }
    }
    async function delFornitore(fid) {
        if (!confirm("Eliminare il fornitore #"+fid+"?")) return;
        try { await apiCall("/api/fornitori/"+fid,"DELETE"); showAlert("Fornitore eliminato"); loadFornitori(); } catch(e) { showAlert(e.message,"err"); }
    }
    ...
    async function loadCatalogo() {
        try {
            ...
            d.data.forEach(r => {
                ...
                h += "<tr><td>" + r.fnome + " <code style=\"font-size:.7rem;color:var(--muted)\">#" + r.fid + "</code></td>";
    ...
    function addCatalogo() {
        openDlg("<button class=\"dlg-close\" onclick=\"closeDlg()\">\u2715</button><h3>Aggiungi voce catalogo</h3><div class=\"ff\"><label>FID Fornitore *</label><input id=\"f-fid\" type=\"number\"/></div><div class=\"ff\"><label>PID Pezzo *</label><input id=\"f-pid\" type=\"number\"/></div><div class=\"ff\"><label>Costo *</label><input id=\"f-cos\" type=\"number\" step=\"0.01\"/></div><div class=\"dlg-actions\"><button class=\"btn-sm\" onclick=\"closeDlg()\">Annulla</button><button class=\"btn-sm btn-accent\" onclick=\"saveCatalogo()\">Salva</button></div>");
    }
    function editCatalogo(fid,pid,costo) {
        openDlg("<button class=\"dlg-close\" onclick=\"closeDlg()\">\u2715</button><h3>Modifica costo (F#"+fid+" / P#"+pid+")</h3><div class=\"ff\"><label>Costo *</label><input id=\"f-cos\" type=\"number\" step=\"0.01\" value=\""+costo+"\"/></div><div class=\"dlg-actions\"><button class=\"btn-sm\" onclick=\"closeDlg()\">Annulla</button><button class=\"btn-sm btn-accent\" onclick=\"saveCatalogo("+fid+","+pid+")\">Salva</button></div>");
    }
    async function saveCatalogo(fid=null,pid=null) {
        try {
            if (fid&&pid) await apiCall("/api/catalogo/"+fid+"/"+pid,"PUT",{costo:parseFloat(document.getElementById("f-cos").value)});
            else await apiCall("/api/catalogo","POST",{fid:parseInt(document.getElementById("f-fid").value),pid:parseInt(document.getElementById("f-pid").value),costo:parseFloat(document.getElementById("f-cos").value)});
            closeDlg(); showAlert(fid?"Costo aggiornato":"Voce aggiunta"); loadCatalogo();
        } catch(e) { showAlert(e.message,"err"); }
    }
    async function delCatalogo(fid,pid) {
        if (!confirm("Rimuovere dal catalogo?")) return;
        try { await apiCall("/api/catalogo/"+fid+"/"+pid,"DELETE"); showAlert("Rimosso dal catalogo"); loadCatalogo(); } catch(e) { showAlert(e.message,"err"); }
    }
    ...
    async function loadUtenti() {
        try {
            ...
            d.data.forEach(u => {
                ...
                const dk = storeObj([["ID",u.uid],["Username",u.username],["Ruolo",u.ruolo],["Fornitore",fLbl]]);
                h += "<tr><td><code>"+u.uid+"</code></td><td>"+u.username+"</td><td><span class=\"tag-pill "+tag+"\">"+u.ruolo+"</span></td><td>"+fLbl+"</td>";
    ...
    function addUtente() {
        openDlg("<button class=\"dlg-close\" onclick=\"closeDlg()\">\u2715</button><h3>Nuovo Utente</h3><div class=\"ff\"><label>Username *</label><input id=\"f-un\" placeholder=\"username\"/></div><div class=\"ff\"><label>Password *</label><input id=\"f-pw\" type=\"password\"/></div><div class=\"ff\"><label>Ruolo *</label><select id=\"f-ro\" onchange=\"toggleFidWrap(this.value)\"><option value=\"fornitore\">Fornitore</option><option value=\"admin\">Admin</option></select></div><div class=\"ff\" id=\"fid-wrap\"><label>FID Fornitore *</label><input id=\"f-fi\" type=\"number\" placeholder=\"ID del fornitore\"/></div><div class=\"dlg-actions\"><button class=\"btn-sm\" onclick=\"closeDlg()\">Annulla</button><button class=\"btn-sm btn-accent\" onclick=\"saveUtente()\">Crea</button></div>");
    }
    ...
    async function saveUtente() {
        ...
        if (ro==="fornitore"&&!fid) { showAlert("FID obbligatorio","err"); return; }
        const b = { username: document.getElementById("f-un").value.trim(), password: document.getElementById("f-pw").value, ruolo: ro, fid: fid?parseInt(fid):null };
        if (!b.username||!b.password) { showAlert("Username e password obbligatori","err"); return; }
        try { await apiCall("/api/auth/register","POST",b); closeDlg(); showAlert("Utente creato"); loadUtenti(); } catch(e) { showAlert(e.message,"err"); }
    }
    async function delUtente(uid) {
        if (!confirm("Eliminare l\'utente #"+uid+"?")) return;
        try { await apiCall("/api/utenti/"+uid,"DELETE"); showAlert("Utente eliminato"); loadUtenti(); } catch(e) { showAlert(e.message,"err"); }
    }
    ...
    async function loadMioCatalogo(fid) {
        try {
            ...
            d.data.forEach(r => {
                ...
                h += "<tr><td>"+r.pnome+" <code style=\"font-size:.7rem;color:var(--muted)\">#"+r.pid+"</code></td>";
                h += "<td>"+(r.colore||"\u2014")+"</td><td>\u20ac"+r.costo+"</td>";
                h += "<td style=\"display:flex;gap:6px\">";
    ...
    function addMioCatalogo(fid) {
        openDlg("<button class=\"dlg-close\" onclick=\"closeDlg()\">\u2715</button><h3>Aggiungi pezzo al catalogo</h3><div class=\"ff\"><label>PID Pezzo *</label><input id=\"f-pid\" type=\"number\" placeholder=\"ID del pezzo\"/></div><div class=\"ff\"><label>Costo *</label><input id=\"f-cos\" type=\"number\" step=\"0.01\" placeholder=\"0.00\"/></div><div class=\"dlg-actions\"><button class=\"btn-sm\" onclick=\"closeDlg()\">Annulla</button><button class=\"btn-sm btn-accent\" onclick=\"saveMioCatalogo("+fid+")\">Salva</button></div>");
    }
    function editMioCatalogo(pid,costo) {
        const fid = window._myFid;
        openDlg("<button class=\"dlg-close\" onclick=\"closeDlg()\">\u2715</button><h3>Modifica costo pezzo #"+pid+"</h3><div class=\"ff\"><label>Nuovo costo *</label><input id=\"f-cos\" type=\"number\" step=\"0.01\" value=\""+costo+"\"/></div><div class=\"dlg-actions\"><button class=\"btn-sm\" onclick=\"closeDlg()\">Annulla</button><button class=\"btn-sm btn-accent\" onclick=\"saveMioCatalogo("+fid+","+pid+")\">Salva</button></div>");
    }
    async function saveMioCatalogo(fid,pid=null) {
        ...
        try {
            if (pid) { await apiCall("/api/catalogo/"+fid+"/"+pid,"PUT",{costo}); showAlert("Costo aggiornato"); }
            else { const pidVal=parseInt(document.getElementById("f-pid").value); if(!pidVal){showAlert("PID non valido","err");return;} await apiCall("/api/catalogo","POST",{fid,pid:pidVal,costo}); showAlert("Pezzo aggiunto al catalogo"); }
            closeDlg(); loadMioCatalogo(fid);
        } catch(e) { showAlert(e.message,"err"); }
    }
    async function delMioCatalogo(fid,pid) {
        if (!confirm("Rimuovere il pezzo #"+pid+" dal tuo catalogo?")) return;
        try { await apiCall("/api/catalogo/"+fid+"/"+pid,"DELETE"); showAlert("Pezzo rimosso"); loadMioCatalogo(fid); } catch(e) { showAlert(e.message,"err"); }
    }
    ';
...
<div class="card">
  ...
  <div class="fg"><label>Password</label><input type="password" id="p" autocomplete="current-password"/></div>
...
async function doLogin() {
    ...
    const e=document.getElementById("e"), btn=document.getElementById("btnLogin");
    e.textContent="";
    ...
    } catch(ex){e.textContent=ex.message;btn.disabled=false;btn.textContent="Accedi";}
}
</script></body></html>
HTML;
    $res->getBody()->write($html);
...
$app->get('/dashboard/admin', function (Request $req, Response $res) {
    ...
    <div id="dash-content">
      <div class="section-header">
        ...
        <p>Gestisci pezzi, fornitori, catalogo e utenti.
      ...
      <div id="tab-pezzi" class="tab-panel"><div class="card"><div class="toolbar"><input class="search-input" placeholder="Cerca per nome..." oninput="pzQ=this.value;pzPg=1;loadPezzi()"/><button class="btn-sm btn-accent" onclick="addPezzo()">+ Aggiungi Pezzo</button></div><div id="pz-tbl"></div></div></div>
      <div id="tab-fornitori" class="tab-panel" style="display:none"><div class="card"><div class="toolbar"><input class="search-input" placeholder="Cerca per nome..." oninput="fnQ=this.value;fnPg=1;loadFornitori()"/><button class="btn-sm btn-accent" onclick="addFornitore()">+ Aggiungi Fornitore</button></div><div id="fn-tbl"></div></div></div>
      <div id="tab-catalogo" class="tab-panel" style="display:none"><div class="card"><div class="toolbar"><button class="btn-sm btn-accent" onclick="addCatalogo()">+ Aggiungi voce</button></div><div id="ca-tbl"></div></div></div>
      <div id="tab-utenti" class="tab-panel" style="display:none"><div class="card"><div class="toolbar"><button class="btn-sm btn-accent" onclick="addUtente()">+ Nuovo Utente</button></div><div id="ut-tbl"></div></div></div>
    </div>
    <script>
    (function(){
        const tok=localStorage.getItem("cat_jwt"), ruolo=localStorage.getItem("cat_ruolo");
        ...
        document.getElementById("btn-logout").addEventListener("click",function(e){e.preventDefault();localStorage.clear();window.location.href="/dashboard/login";});
    })();
...
$app->get('/dashboard/fornitore', function (Request $req, Response $res) {
    ...
    <div id="dash-content">
      <div class="section-header">
        ...
        <p>Gestisci i pezzi del tuo catalogo.
           <span id="user-info" style="color:var(--accent);font-weight:700"></span>
           <a href="/dashboard/login" id="btn-logout" style="color:var(--muted);font-size:.8rem;margin-left:12px">Esci &rarr;</a>
        </p>
      </div>
      <div class="card">
        <div class="toolbar"><button class="btn-sm btn-accent" id="btn-add">+ Aggiungi pezzo</button></div>
        <div id="mc-tbl"></div>
      </div>
    </div>
    <script>
    (function(){
        const tok=localStorage.getItem("cat_jwt"), ruolo=localStorage.getItem("cat_ruolo"), fid=parseInt(localStorage.getItem("cat_fid"));
...
// ══════════════════════════════════════════════════════════════════════════
// ROUTE FRONTEND — REDIRECT ROOT + 10 QUERY PAGES
// ══════════════════════════════════════════════════════════════════════════
...
$app->get('/frontend/q1', function (Request $req, Response $res) {
    ...
    $c .= '<button class="btn-fetch" onclick="fetchData(\'/api/1\', this, \'result\')">&#9654; Esegui Query</button><div class="result-area" id="result"></div>';
...
$app->get('/frontend/q2', function (Request $req, Response $res) {
    ...
    $c .= configBar() . '<div class="sql-block">' . htmlspecialchars($sql) . '</div>';
    $c .= '<button class="btn-fetch" onclick="fetchData(\'/api/2\', this, \'result\')">&#9654; Esegui Query</button><div class="result-area" id="result"></div>';
...
$app->get('/frontend/q3', function (Request $req, Response $res) {
    ...
    $c .= configBar() . '<div class="sql-block">' . htmlspecialchars($sql) . '</div>';
    $c .= '<button class="btn-fetch" onclick="fetchData(\'/api/3\', this, \'result\')">&#9654; Esegui Query</button><div class="result-area" id="result"></div>';
...
$app->get('/frontend/q4', function (Request $req, Response $res) {
    ...
    $c .= configBar() . '<div class="sql-block">' . htmlspecialchars($sql) . '</div>';
    $c .= '<button class="btn-fetch" onclick="fetchData(\'/api/4\', this, \'result\')">&#9654; Esegui Query</button><div class="result-area" id="result"></div>';
...
$app->get('/frontend/q5', function (Request $req, Response $res) {
    ...
    $c = '<div class="section-header"><div class="tag">Query 05</div><h2>Fornitori con costo sopra la media</h2><p>Fornitori con almeno un pezzo con costo superiore alla media.</p></div>';
    $c .= configBar() . '<div class="sql-block">' . htmlspecialchars($sql) . '</div>';
    $c .= '<button class="btn-fetch" onclick="fetchData(\'/api/5\', this, \'result\')">&#9654; Esegui Query</button><div class="result-area" id="result"></div>';
...
$app->get('/frontend/q6', function (Request $req, Response $res) {
    ...
    $c .= configBar() . '<div class="sql-block">' . htmlspecialchars($sql) . '</div>';
    $c .= '<button class="btn-fetch" onclick="fetchData(\'/api/6\', this, \'result\')">&#9654; Esegui Query</button><div class="result-area" id="result"></div>';
...
$app->get('/frontend/q7', function (Request $req, Response $res) {
    ...
    $c .= configBar() . '<div class="sql-block">' . htmlspecialchars($sql) . '</div>';
    $c .= '<button class="btn-fetch" onclick="fetchData(\'/api/7\', this, \'result\')">&#9654; Esegui Query</button><div class="result-area" id="result"></div>';
...
$app->get('/frontend/q8', function (Request $req, Response $res) {
    ...
    $c .= configBar() . '<div class="sql-block">' . htmlspecialchars($sql) . '</div>';
    $c .= '<button class="btn-fetch" onclick="fetchData(\'/api/8\', this, \'result\')">&#9654; Esegui Query</button><div class="result-area" id="result"></div>';
...
$app->get('/frontend/q9', function (Request $req, Response $res) {
    ...
    $c .= configBar() . '<div class="sql-block">' . htmlspecialchars($sql) . '</div>';
    $c .= '<button class="btn-fetch" onclick="fetchData(\'/api/9\', this, \'result\')">&#9654; Esegui Query</button><div class="result-area" id="result"></div>';
...
$app->get('/frontend/q10', function (Request $req, Response $res) {
    ...
    $c .= configBar() . '<div class="sql-block">' . htmlspecialchars($sql) . '</div>';
    $c .= '<button class="btn-fetch" onclick="fetchData(\'/api/10\', this, \'result\')">&#9654; Esegui Query</button><div class="result-area" id="result"></div>';
